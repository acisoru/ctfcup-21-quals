...
int main()
{
    ...
    int nbytes = read(0, flag, 28);
    flag[27] = '\0';

    for (int i = 0; i < 27; i++) {
        int* path = GetPathToChar(root, flag[i]);
...
int checkMd5Hash(char* flag)
{
    uint8_t hash[16] = {0x9d, 0xb9, 0x29, 0xbc, 0x46, 0x39, 0xcf, 0xf9, 0xae, 0xf5, 0xb3, 0x85, 0x03, 0x22, 0xd2, 0x9b};
    uint8_t* result = md5String(flag);
    return memcmp(result, hash, 16);
};
